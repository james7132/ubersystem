...
def checkins_by_hour_query(session):
    return session.execute(select(date_trunc_hour(Attendee.checked_in),
...
class Root:
    def comped_badges(self, session, message='', show='all'):
        ...
        all_comped_stmt = regular_comped.union(promo_comped, group_comped)
        all_comped = session.scalars(all_comped_stmt).all()
        claimed_comped = [a for a in all_comped if not a.placeholder]
        unclaimed_comped = [a for a in all_comped if a.placeholder]

        return {
            'message': message,
            'comped_attendees': all_comped,
            'all_comped': len(all_comped),
            'claimed_comped': len(claimed_comped),
            'unclaimed_comped': len(unclaimed_comped),
            'show': show,
        }

    @csv_file
    def comped_badges_csv(self, out, session):
        comped = session.scalars(select(Attendee).filter(
    ...
    def found_how(self, session):
        return {'all': sorted(
            [a.found_how for a in session.scalars(select(Attendee).filter(Attendee.found_how != '')).all()],
        ...
        attendees = session.scalars(select(Attendee).filter(
            filter).join(Attendee.active_receipt).outerjoin(ModelReceipt.receipt_items).group_by(
                ModelReceipt.id).group_by(Attendee.id).having(
                    Attendee.default_cost_cents != ModelReceipt.fkless_item_total_sql).options(lazyload("*"))).all()
    ...
    def attendees_nonzero_balance(self, session, include_no_receipts=False, include_discrepancies=False):
        ...
        attendees_and_totals = session.execute(select(
        ...
        if include_no_receipts:
            attendees_no_receipts = session.scalars(select(Attendee).outerjoin(
                ModelReceipt, Attendee.active_receipt).filter(Attendee.default_cost > 0, ModelReceipt.id == None)).all()
        else:
    ...
    def self_service_refunds(self, session):
        refunds = session.scalars(select(ReceiptTransaction).filter(ReceiptTransaction.amount < 0,
    ...
    def checkins_by_admin_by_hour(self, out, session):
        ...
        admin_list = []
        admins = session.execute(select(Tracking.who).filter(Tracking.action == c.UPDATED,
        ...
        hour_admin_checkins = session.execute(select(
            ...
            date_trunc_hour(Tracking.when))).all()
